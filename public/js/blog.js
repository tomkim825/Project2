    ...
    $.get("/api/posts" + authorId, function(data) {
      ...
      if (!posts || !posts.length) {
        ...
        for (var k =0; k < globalTags.length; k++){
          $('<h2>').text("#"+globalTags[k])
          .addClass('textBubbles').css('top', 5.2*Math.pow(k,3) + Math.floor(Math.random()*10) - 100 +"px").css('left', 1700 * k +'px').appendTo("#test2");
        }
      }
    });
  }
...
//========================================  
  ...
  function createNewRow(post,i) {
    ...
    for (var j =0; j < tagArray.length; j++) {
      console.log(tagArray[j]);
      tagString += ' #' + tagArray[j];
    }
    // console.log(tagArray);
    if (tagArray.length === 1) {
      globalTags.push(tagArray[0])
      console.log(globalTags);
    } else {

      for (var i = 0 ; i < tagArray.length ;  i++) {
       
        globalTags.push(tagArray[i]);
      ...
      // $('<h2>').text(tagArray[i])
      // .addClass('textBubbles').css('top', 5.2*Math.pow(i,3) + Math.floor(Math.random()*10) - 650 +"px").css('left', 900 * i +'px').appendTo("#test2");
    // };
